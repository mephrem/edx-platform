...
def dump_js_escaped_json(obj, cls=EdxJSONEncoder):
    ...
    if isinstance(obj, dict) and 'courseId' in obj:
        obj['courseId'] = obj.get('courseId').decode('utf-8') if type(obj.get('courseId')) == bytes else obj.get('courseId')
    if isinstance(obj, dict) and 'usageId' in obj:
        obj['usageId'] = obj.get('usageId').decode('utf-8') if type(obj.get('usageId')) == bytes else obj.get('usageId')
